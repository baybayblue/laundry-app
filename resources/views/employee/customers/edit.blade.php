...
<div class="row g-4">
    ...
    <div class="col-12 col-lg-8">
        <form action="{{ route('employee.customers.update', $customer) }}" method="POST" id="customerForm" novalidate>
            @csrf
            @method('PUT')
        ...
        <div class="card border-0 shadow-sm rounded-4 mb-3">
            <div class="card-body p-4">
                <h6 class="fw-semibold mb-3 d-flex align-items-center gap-2 text-primary">
                    <i class="ti ti-user-check"></i> Info Pelanggan
                </h6>
        ...
        <div class="card border-0 shadow-sm rounded-4" style="background: linear-gradient(135deg, #dc354508, #fff);">
            <div class="card-body p-4">
                <h6 class="fw-semibold mb-2 d-flex align-items-center gap-2 text-danger">
                    <i class="ti ti-alert-triangle"></i> Operasi Sensitif
                </h6>
                <p class="small text-muted mb-3">Penghapusan data pelanggan akan mencabut akses login mereka jika ada.</p>
                <form action="{{ route('employee.customers.destroy', $customer) }}" method="POST" class="form-delete-sidebar">
                    @csrf
                    @method('DELETE')
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (btnDeleteSidebar) {
            btnDeleteSidebar.addEventListener('click', function () {
                const formDelete = document.querySelector('.form-delete-sidebar');
                Swal.fire({
